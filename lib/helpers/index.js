...
function isPathMatchAnyPattern(path, patterns) {
  if (typeof path !== 'string' || !Array.isArray(patterns) || patterns.length === 0) {
    return false;
  }
...
function isRelativePathCrossSlice(currentFilePath, relativePath, layers, layersConfig = {}) {
  ...
  // Check if current layer is a valid FSD layer
  // This prevents checking imports in non-FSD folders like i18n, any other folders at src/ level
  if (!layerValues.includes(currentLayer)) return false;
  ...
  // Check if resolved layer is a valid FSD layer
  // This prevents checking imports to non-FSD folders like i18n, any other folders at src/ level
  if (!layerValues.includes(resolvedLayer)) return false;
